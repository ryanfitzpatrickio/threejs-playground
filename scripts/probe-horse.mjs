#!/usr/bin/env node
/**
 * probe-horse — deterministic screenshots of the bespoke procedural horse
 * ("horse v2", catalog id domestic-horse-procedural) in the dog studio
 * harness, for look-dev + to surface WebGPU/TSL shader compile errors that
 * only appear on a real GPU (not headless node).
 *
 * Usage:
 *   node scripts/probe-horse.mjs [presetId ...] [--naked] [--behavior=trot]
 *       [--compare] [--live] [--breed=domestic-horse-procedural]
 *
 * Only the horse-rig catalog entry builds the bespoke procedural horse in the
 * harness; `domestic-horse` (v1) renders the dog-derived phenotype.
 *
 * Requires the dev server (default http://127.0.0.1:5173/). PNGs → .codex-tmp/horse/.
 */
...
const breed = args.find((a) => a.startsWith('--breed='))?.slice('--breed='.length) ?? 'domestic-horse-procedural';
const presets = args.filter((a) => !a.startsWith('--'));
if (presets.length === 0) presets.push('profile', 'three-quarter', 'head-close', 'walk-side');

const url = dreamfallAppUrl({ view: 'dog-sim', harness: '1', autostart: null });
const outDir = path.resolve('.codex-tmp', 'horse');
...
try {
  ...
  // Harness: render-vs-photo compare pane wires to the equid-ref board.
  if (compare) {
  ...
  // Harness: live-motion pass — run through the procedural gait catalog.
  if (live) {
    ...
    for (const clip of ['Walk', 'Trot', 'Canter', 'Gallop', 'Graze', 'Rest']) {
      ...
      const file = path.join(outDir, `horse-${breed}-live-${clip.toLowerCase()}.png`);
  ...
  for (const preset of presets) {
    ...
    const canvas = await page.$('canvas');
    const file = path.join(outDir, `horse-${breed}-${preset}${naked ? '-naked' : ''}${behavior ? `-${behavior}` : ''}.png`);
    if (canvas) await canvas.screenshot({ path: file });
