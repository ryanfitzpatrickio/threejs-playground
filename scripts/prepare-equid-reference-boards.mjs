#!/usr/bin/env node
/**
 * Slice standardized 2x2 equid reference boards into optimized scene images.
 *
 * Source: assets-source/equid-ref/<breed-id>/board.(png|jpg|webp)
 * Quadrants (reading order): three-quarter, profile, front-sit, head-close.
 * For horses, "front-sit" is a front-facing standing conformation view
 * (ungulates do not sit like canids).
 *
 * Usage: npm run prepare:equid-ref [-- <breed-id> ...]
 */
...
const SOURCE_ROOT = path.resolve('assets-source/equid-ref');
const OUTPUT_ROOT = path.resolve('public/assets/equid-ref');
const VIEW_NAMES = ['three-quarter', 'profile', 'front-sit', 'head-close'];

function isEquidaeBreed(breed) {
  return breed?.speciesId === 'equidae'
    || breed?.familyId === 'equid'
    || Boolean(breed?.conformationFlags?.includes('equidae'));
}

const EQUID_BREED_IDS = DOG_BREEDS
  .filter((breed) => isEquidaeBreed(breed))
  .map((breed) => breed.id);
const requested = process.argv.slice(2).filter((arg) => !arg.startsWith('--'));
const targets = requested.length ? requested : EQUID_BREED_IDS;
...
for (const breedId of targets) {
  ...
  if (!breed || !isEquidaeBreed(breed)) {
    throw new Error(`Unknown equidae breed: ${breedId}`);
  }
...
console.log(`ok — prepared ${prepared} equid reference board${prepared === 1 ? '' : 's'}`);
