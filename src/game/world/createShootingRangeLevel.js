 ...
 * ~4× floor area (≈28×110 m), gabled roof, clerestory window band, and
 * additive light shafts (god rays) from sky light through the upper glass.
 */
...
const godRayHotMat = new THREE.MeshBasicMaterial({
  color: 0xfff2d0,
  transparent: true,
  opacity: 0.14,
  blending: THREE.AdditiveBlending,
...
export function createShootingRangeLevel() {
  ...
  const colliders = [];
  const materials = [
    floorMat, plankMat, darkTimberMat, beamMat, wallBoardMat,
    crateMat, palletMat, metalMat, dangerStripeMat, friendlyStripeMat,
    roofMat, roofUndersideMat, glassMat, mullionMat, godRayMat, godRayHotMat, dustMoteMat,
  ];
  ...
  // ── God rays / dust shafts from high windows ─────────────────────────────
  addGodRays({ group, halfW });
  ...
  // Dim ambient — volume is lit by clerestory glow + god rays + scene sun/env.
...
function addExteriorWalls({ group, colliders, halfW, centerZ, length }) {
  // Solid boarded walls only up to SOLID_WALL_H — clerestory sits above.
  for (const side of [-1, 1]) {
    ...
    addBox({
      ...
      sy: SOLID_WALL_H,
      sz: length + WALL_T * 2,
      material: wallBoardMat,
      collider: true,
    });
    // Board courses on solid face
    for (let row = 0; row < 4; row += 1) {
      addBox({
        ...
        sy: 0.85,
        sz: length + WALL_T,
        material: row % 2 === 0 ? plankMat : wallBoardMat,
        collider: false,
      });
    }
  ...
  for (const z of [COURSE_MIN_Z, COURSE_MAX_Z]) {
    for (const side of [-1, 1]) {
      const panel = new THREE.Mesh(
        new THREE.BoxGeometry(halfW - 0.2, 0.16, WALL_T * 0.9),
        wallBoardMat,
      );
...
function addClerestoryWindows({ group, halfW, centerZ, length }) {
  ...
  for (const side of [-1, 1]) {
    ...
    for (let z = COURSE_MIN_Z + 2; z < COURSE_MAX_Z - 2; z += bay) {
      ...
      // A clustered light for every pane created 56 local lights. One wider fill
      // per three bays overlaps its neighbours and keeps the same warm window
      // rhythm with one third of the cluster/light-upload work.
      if (bayIndex % 3 === 1) {
        const fill = new THREE.PointLight(0xffe8c0, 7.2, 26, 1.8);
        fill.name = `Clerestory Fill ${side}_${bayIndex}`;
        fill.position.set(side * (halfW - 1.2), winY - 0.4, cz);
...
/**
 * Volumetric-looking god rays: additive cones/planes from clerestory down sun dir,
 * plus a sprinkle of dust motes in the shafts.
 */
function addGodRays({ group, halfW }) {
  const rayGroup = new THREE.Group();
  rayGroup.name = 'God Rays';
  group.add(rayGroup);

  const winY = SOLID_WALL_H + CLERESTORY_H * 0.55;
  // March direction into the hall (down + toward center, biased by sun).
  const dir = SUN_DIR.clone().normalize();
  const rayLen = 11.5;

  let rayIndex = 0;
  for (const side of [-1, 1]) {
    // Prefer the sunward long wall (west is more lit with our sun bias).
    if (side > 0 && rayIndex > 0) {
      // Still add fewer rays on east
    }
    const step = side < 0 ? POST_SPACING : POST_SPACING * 1.5;
    for (let z = COURSE_MIN_Z + 6; z < COURSE_MAX_Z - 6; z += step) {
      const origin = new THREE.Vector3(
        side * (halfW - 0.35),
        winY,
        z + (side < 0 ? 0 : 1.2),
      );
      // Bend slightly toward hall center so shafts read indoors.
      const localDir = dir.clone();
      localDir.x += -side * 0.35;
      localDir.normalize();

      const hot = side < 0 && (rayIndex % 2 === 0);
      addLightShaft({
        parent: rayGroup,
        origin,
        direction: localDir,
        length: rayLen * (hot ? 1.15 : 0.95),
        startRadius: hot ? 1.35 : 0.95,
        endRadius: hot ? 3.4 : 2.4,
        material: hot ? godRayHotMat : godRayMat,
        name: `Shaft ${rayIndex}`,
      });

      // Secondary thinner shaft for layered density
      if (hot) {
        const d2 = localDir.clone();
        d2.x += -side * 0.08;
        d2.z += 0.12;
        d2.normalize();
        addLightShaft({
          parent: rayGroup,
          origin: origin.clone().add(new THREE.Vector3(0, -0.15, 0.4)),
          direction: d2,
          length: rayLen * 0.85,
          startRadius: 0.55,
          endRadius: 1.8,
          material: godRayMat,
          name: `Shaft Soft ${rayIndex}`,
        });
      }
  ...
  // A few shafts from the south entry gable clerestory
  for (let i = 0; i < 3; i += 1) {
    const origin = new THREE.Vector3(-6 + i * 6, winY, COURSE_MIN_Z + 0.5);
    const d = dir.clone();
    d.z = Math.abs(d.z) + 0.25;
    d.normalize();
    addLightShaft({
      parent: rayGroup,
      origin,
      ...
      startRadius: 1.1,
      endRadius: 2.8,
      material: i === 1 ? godRayHotMat : godRayMat,
      name: `Entry Shaft ${i}`,
    });
  }
}
...
function addLightShaft({
  ...
  // Truncated cone along +Y, then reorient to `direction`.
  ...
  // Cylinder default axis is Y; align +Y with direction.
  const up = new THREE.Vector3(0, 1, 0);
  const quat = new THREE.Quaternion().setFromUnitVectors(up, direction.clone().normalize());
  mesh.quaternion.copy(quat);
  // Place so top (startRadius, -length/2 in local before rot) sits at origin...
  // After rotation, center is origin + direction * (length * 0.5)
  mesh.position.copy(origin).addScaledVector(direction, length * 0.5);
  parent.add(mesh);
  return mesh;
}

function addDustMotes({ parent, origin, direction, length, count, seed }) {
...
function buildInteriorLayout({ group, colliders, halfW }) {
  // Bay A partial dividers (z ~ 12–28)
  addPartitionWall({
    group, colliders,
  ...
  for (let i = 0; i < mazeWalls.length; i += 1) {
    ...
    addBox({
      ...
      sy: w.h,
      sz: w.sz,
      material: i % 2 === 0 ? plankMat : wallBoardMat,
      collider: true,
    });
  }
...
function addPartitionWall({
  group, colliders, x0, x1, z, h, doorX = null, doorW = 2.3,
}) {
  const mid = (x0 + x1) * 0.5;
  const fullW = x1 - x0;
  if (fullW < 0.3) return;
