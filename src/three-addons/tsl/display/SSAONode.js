// Vendored from three.js (https://github.com/mrdoob/three.js)
// dev branch commit 6f583581cc (2026-07-01), examples/jsm/tsl/display/SSAONode.js.
// three.js is MIT licensed — Copyright © 2010-2026 three.js authors.
//
// r185 adaptation: `getScreenPositionFromClip` is not exported by three/tsl in
// r185, so it is inlined below (copied from src/nodes/utils/PostProcessingUtils.js
// at the same commit). No other changes.
import { RenderTarget, Vector2, TempNode, QuadMesh, NodeMaterial, RendererUtils, RedFormat } from 'three/webgpu';
import { reference, logarithmicDepthToViewZ, viewZToPerspectiveDepth, getViewPosition, vogelDiskSample, interleavedGradientNoise, nodeObject, Fn, float, NodeUpdateType, uv, uniform, Loop, vec2, vec4, int, dot, max, clamp, length, screenCoordinate, PI2, texture, passTexture } from 'three/tsl';
import { depthAwareBlur } from './depthAwareBlur.js';

/**
 * Computes screen position from a clip position, with WebGPU-style flipped y.
 * Inlined from upstream `PostProcessingUtils.js` (not exported in three r185).
 *
 * @tsl
 * @function
 * @param {Node<vec4>} clipPosition - The clip position.
 * @return {Node<vec2>} The screen position.
 */
